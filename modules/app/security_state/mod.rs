...
///
/// `false` — this graph is one process holding one ledger, and a
/// single-node authority genuinely may take `LocalDurable`: it has no other
/// replica to lose the write to. Forcing `ReplicatedDurable` here would
/// refuse every write in a deployment that is correct.
///
/// It is a constant rather than a parameter because getting it wrong in the
/// permissive direction is silent: a replicated deployment that left this
/// false would accept a `LocalDurable` ack for a revocation and lose it
/// with the node. A replicated deployment changes this and the graph that
/// backs it in the same commit, which is the point at which someone is
/// thinking about it.
const REPLICATED_AUTHORITY: bool = false;
...
unsafe fn handle_request(s: &mut ModuleState, sys: &SyscallTable, msg_type: u8, plen: usize) {
    ...
    match msg_type {
        state_wire::MSG_STATE_GET => {
            ...
            do_get(s, sys, key_len, corr, client);
        }
...
///
/// Caller holds an exclusive `&mut ModuleState` and a live `SyscallTable`.
unsafe fn do_get(s: &mut ModuleState, sys: &SyscallTable, key_len: usize, corr: u32, client: u8) {
...
unsafe fn do_write(
    ...
    if !state_wire::fence_satisfies(namespace, achieved, REPLICATED_AUTHORITY) {
        s.state_unavailable = s.state_unavailable.saturating_add(1);
        reply(s, sys, corr, client, auth_wire::ST_UNAVAILABLE, &[], false);
        return;
    }
